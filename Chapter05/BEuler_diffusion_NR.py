#!/usr/bin/env python3
"""
Diffusion with Neumann or Robin BCs via Backward Euler

We approximate the solution of the Diffusion Equation (5.37) -- (5.38) with
using either Neumann or Robin boundary conditions using Backward Euler
...
# =============================================================================
def BE_solve_NR(x, t, uinit, D, BC0, BCL):
	dx = x[1] - x[0]
	dt = t[1] - t[0]
	Nx = len(x) - 1
	Nt = len(t) - 1
...
def BEuler_Diffusion_NR():
	...
	U = BE_solve_NR(x, t, u0_profile, D, BC0, BCL)
	doMovie(x, t, U, 2**3)
